...
const Form = () => {
  const { addToTask, taskUpdate, updateTask, initialValues, setTaskUpdate } = useTask();
  
  const formik = useFormik({
    initialValues,
    validationSchema,
    onSubmit: (values, { resetForm }) => {
      taskUpdate?.id ? updateTask(values) : addToTask({...values, id: new Date().getTime()});
      setTaskUpdate();
      resetForm();
    },
  })
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="text-center justify-center">
      <div className="flex">
        ...
        {formik.touched.priority && formik.errors.priority && <div>{formik.errors.priority}</div>}
